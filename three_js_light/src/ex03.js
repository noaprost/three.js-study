...
// ----- 주제: PointLight
/**
 * PointLight(색, 강도, , 거리에 따라서 얼마나 빛이 약해지게 할 것인지)
 * 어떠한 지점에 사방으로 빛이 퍼지는 전구 같은 느낌의 빛
 * distance : 빛이 영향을 미치는 거리(기본 값은 0(제한 없음))
 * decay : 거리에 따라서 얼마나 빛이 약해지게 할 것인지
 * 기본 값은 1, 실제와 비슷한 느낌을 주려면 2로 설정하는 것을 추천
 */
...
export default function example() {
  ...
  const light = new THREE.PointLight("white", 15, 100, 2);
  light.position.x = -5;
  light.position.y = 3;
  scene.add(light);
